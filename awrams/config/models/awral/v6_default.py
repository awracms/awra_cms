...
def get_settings(sys_settings=None):
    ...
    profile = {
        'SPATIAL_FILE': _join(model_data_path, 'awral/spatial_parameters_v6.h5'),
        'PARAMETER_FILE': _join(model_config_path, 'parameters/DefaultParameters_v6.json'),
...
def get_input_mapping(model_settings=None):
    ...
    SPATIAL_GRIDS = ['f_tree', 'height', 'hveg_dr', 'k0sat', 'k_gw', 'kdsat', 'kssat', 'lai_max',
                     'meanPET', 'ne', 'pref', 's0fracAWC', 'slope', 'ssfracAWC', 'sdfracAWC']
    ...
    for k in ['tmin', 'tmax', 'precip', 'solar', 'wind']:
    ...
    mapping['sdmax'] = nodes.mul('sdmax_scale', 'sdfracawc_grid', 5000.)

    mapping['k0sat'] = nodes.mul('k0sat_scale', 'k0sat_grid')
    mapping['kssat'] = nodes.mul('kssat_scale', 'kssat_grid')
    mapping['kdsat'] = nodes.mul('kdsat_scale', 'kdsat_grid')
